      ...
      <main className="container mx-auto px-6 pt-32 pb-20">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {posts.length > 0 ? (
              posts.map((post: any) => (
                <article key={post.id} className="bg-white/5 border border-white/10 p-6 rounded-xl hover:border-[#00ff00]/50 transition-all group">
                  <div className="flex flex-wrap gap-2 mb-4">
                    {post.labels && post.labels.map((label: any, index: number) => (
                      <span key={index} className="text-[10px] bg-[#00ff00]/10 text-[#00ff00] px-2 py-1 rounded">
                        #{typeof label === 'string' ? label : label.name}
                      </span>
                    ))}
                  </div>
                  <h2 className="text-xl font-bold mb-4 line-clamp-2 group-hover:text-[#00ff00] transition-colors">{post.title}</h2>
