...
const navLinks = [
  ...
  { label: "Blog", href: "https://blog.line88.tw/" }, // 已補回
      ...
      <div className="max-w-7xl mx-auto px-6 h-full flex items-center justify-between relative">
        ...
        >
          <div className="w-6 flex flex-col gap-1.5 pointer-events-none">
            <span className={`h-0.5 w-full bg-[#00ff00] transition-all duration-300 ${mobileOpen ? "rotate-45 translate-y-2" : ""}`} />
            <span className={`h-0.5 w-full bg-[#00ff00] transition-all duration-300 ${mobileOpen ? "opacity-0" : ""}`} />
            <span className={`h-0.5 w-full bg-[#00ff00] transition-all duration-300 ${mobileOpen ? "-rotate-45 -translate-y-2" : ""}`} />
          </div>
        </button>
      </div>

      {/* Mobile Dropdown：優化動畫與背景遮罩 */}
      {mobileOpen && (
        <div 
        ...
        >
          {navLinks.map((link) => (
            <a
              ...
              className="px-8 py-6 text-xl font-bold text-gray-200 border-b border-white/5 active:bg-[#00ff00]/10 hover:text-[#00ff00] transition-colors"
            >
              {link.label}
            </a>
          ))}
